  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf("image/") == -1) {
        this.$message.error(
          "The file format is incorrect. Please upload an image of the same type, such as a file with a JPG or PNG suffix."
        );
      } else {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob((data) => {
        ...
        userInfoApi.uploadAvatar(formData).then((response) => {
          ...
          this.$message.success("Modification successful");   //调用Element库中的内置对象方法弹出成功的提示框
          this.visible = false;

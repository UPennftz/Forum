...
@ApiResource(name = "RoleManagement" , resBizType = ResBizTypeEnum.SYSTEM)
public class SysRoleController {
    ...
    public R add(@Validated @RequestBody SysRoleEntity role) {
        if (!roleService.checkRoleNameUnique(role)) {
            return R.error("Failed to add the new role'" + role.getRoleName() + "', role name already exists");
        } else if (!roleService.checkRoleKeyUnique(role)) {
            return R.error("Failed to add role'" + role.getRoleName() + "', role permissions already exist");
        }
    ...
    public R edit(@Validated @RequestBody SysRoleEntity role) {
        ...
        if (!roleService.checkRoleNameUnique(role)) {
            return R.error("Failed to modify'" + role.getRoleName() + "', role name already exists.");
        } else if (!roleService.checkRoleKeyUnique(role)) {
            return R.error("Failed to modify'" + role.getRoleName() + "', role permission already exists.");
        }
        ...
        return R.error("Failed to modify'" + role.getRoleName() + "', please contact the administrator");
    }
    ...
    public void export(HttpServletResponse response) throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            headerRow.createCell(2).setCellValue("Key");
            headerRow.createCell(3).setCellValue("Sort");
            headerRow.createCell(4).setCellValue("MenuCheckStrictly");
            ...
            for (SysRoleEntity role : roles) {
                ...
                row.createCell(4).setCellValue(role.isMenuCheckStrictly() ? "Yes" : "No");

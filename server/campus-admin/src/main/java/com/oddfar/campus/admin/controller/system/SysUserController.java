...
@ApiResource(name = "UserManagement", resBizType = ResBizTypeEnum.SYSTEM)
...
//        userService.checkUserAllowed(user);
        if (!(userService.checkUserNameUnique(user))) {
            return R.error("Modify User'" + user.getUserName() + "'failed, the login account already exists");
        } else if (StringUtils.isNotEmpty(user.getPhonenumber())
                && !(userService.checkPhoneUnique(user))) {
            return R.error("Modify User'" + user.getUserName() + "'failed, ID number already exists");
        } else if (StringUtils.isNotEmpty(user.getEmail())
                && !(userService.checkEmailUnique(user))) {
            return R.error("Modify User'" + user.getUserName() + "'failed, the email account already exists");
        }
    ...
    public R insertAuthRole(Long userId, Long[] roleIds) {
        if (!SysUserEntity.isAdmin(userId)) {
            ...
            return R.error("Unable to operate super administrator");
        }
    ...
    public void export(HttpServletResponse response) throws IOException {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            headerRow.createCell(4).setCellValue("IDNumber");
            ...
            headerRow.createCell(8).setCellValue("LastIP");
            headerRow.createCell(9).setCellValue("LastLoginTime");

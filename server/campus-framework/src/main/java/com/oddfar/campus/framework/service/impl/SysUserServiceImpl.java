...
public class SysUserServiceImpl implements SysUserService {
    ...
    public int insertUser(SysUserEntity user) {
        if (StringUtils.isNotEmpty(user.getUserName())
            ...
            throw new ServiceException("Failed to add new user'" + user.getUserName() + "', the login account already exists");
        }
        if (StringUtils.isNotEmpty(user.getPhonenumber())
                && !(checkPhoneUnique(user))) {
            throw new ServiceException("Failed to add new user'" + user.getUserName() + "', ID number already exists");
        }
        if (StringUtils.isNotEmpty(user.getEmail())
                && !(checkEmailUnique(user))) {
            throw new ServiceException("Failed to add new user'" + user.getUserName() + "', email already exists");
        }
